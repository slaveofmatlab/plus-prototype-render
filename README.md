...
- `detail_words_dict.xlsx`：239 个属性词，10 个品类（按长度降序排列用于最长匹配）
- `Brand_words_document.xlsx`：品牌词库（从 Excel 加载）

### 特殊客户预处理 (香格里拉 XGLL)

位于 `Special Orders Process/XGLL/process_orders_xgll.py`，专门处理香格里拉酒店集团的中英双语订单描述。
...
独立于排序分数的 0-1 置信度计算，三个加权维度：

| 维度 | 权重 | 评估方式 |
|------|------|---------|
| 品牌 | 0.5 | 完全匹配 1.0，无品牌双方 0.8，查询有品牌候选无 0.0 |
| 规格 | 0.2 | 完全匹配 1.0，数字匹配 0.7，查询有规格候选无 0.0 |
| 核心名 | 0.3 | 1 - edit_distance / max_len，编辑距离归一化 |

前端根据置信度显示 badge：高(≥0.7) / 中(≥0.4) / 低(<0.4) / 无匹配。
...
| customer_name | string | 客户名称（用于特殊客户检测） |
...
**Step 2 - 品类与基准价**：Step 1 摘要折叠、9 大品类复选框、品类×采购基准价模式矩阵（市调价格/参考现客户价格/网站价格/采购价格）、产品明细表（支持文件导入AI匹配、产品选择器弹窗、手动录入）、毛利汇总。
